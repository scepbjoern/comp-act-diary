-- =============================================================================
-- DATEN-MIGRATION: Nur Kern-Daten (TimeBox, JournalEntry, MediaAsset)
-- =============================================================================
-- Ausführung VOR Prisma db push
-- =============================================================================
...
-- =============================================================================
-- VORBEREITUNG: Neue Tabellen erstellen
-- =============================================================================
...
-- JournalEntryType erstellen (System-Typen)
...
-- =============================================================================
-- SCHRITT 1: Temporäre Mapping-Tabellen erstellen
-- =============================================================================

-- Mapping alte DayEntry.id → neue TimeBox.id
...
-- Mapping alte DayNote.id → neue JournalEntry.id
...
-- =============================================================================
-- SCHRITT 2: Entity-Registry für Habits
-- =============================================================================
...
-- =============================================================================
-- SCHRITT 3: TimeBox erstellen (aus alten DayEntry)
-- =============================================================================
...
-- =============================================================================
-- SCHRITT 4: Custom TimeBoxes für Phase/CareCategory
-- =============================================================================
...
-- =============================================================================
-- SCHRITT 5: DayEntry mit TimeBox verknüpfen (temporär)
-- =============================================================================
...
-- =============================================================================
-- SCHRITT 6: JournalEntryType Seed-Daten
-- =============================================================================
...
-- =============================================================================
-- SCHRITT 7: JournalEntry aus DayNote
-- =============================================================================
...
-- =============================================================================
-- SCHRITT 8: JournalEntry aus Reflection
-- =============================================================================
...
-- =============================================================================
-- SCHRITT 9: MediaAsset aus AudioFile
-- =============================================================================
...
-- =============================================================================
-- SCHRITT 10: MediaAsset aus PhotoFile
-- =============================================================================
...
-- =============================================================================
-- SCHRITT 11: MediaAttachment erstellen
-- =============================================================================
...
JOIN "JournalEntry" je ON je.id = (
  ...
  FROM journal_entry_mapping 
  WHERE old_day_note_id = dn.id
);
...
SELECT 
  gen_random_uuid()::text,
  pf.id,
  COALESCE(je.id, je2.id),
  COALESCE(je."userId", je2."userId"),
  'ATTACHMENT',
  0,
  COALESCE(je."timeBoxId", je2."timeBoxId"),
  NOW()
FROM "PhotoFile" pf
LEFT JOIN "DayNote" dn ON dn.id = pf."dayNoteId"
...
-- Validierung
SELECT 'Kern-Migration abgeschlossen!' as status;
SELECT 
  'TimeBox' as table_name, COUNT(*) as row_count FROM "TimeBox"
UNION ALL
SELECT 'JournalEntry', COUNT(*) FROM "JournalEntry"
UNION ALL
SELECT 'MediaAsset', COUNT(*) FROM "MediaAsset"
UNION ALL
SELECT 'MediaAttachment', COUNT(*) FROM "MediaAttachment"
UNION ALL
