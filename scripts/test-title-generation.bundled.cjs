...
var AbstractChatCompletionRunner = class {
  constructor() {
    ...
    __classPrivateFieldSet(this, _AbstractChatCompletionRunner_connectedPromise, new Promise((resolve2, reject) => {
      __classPrivateFieldSet(this, _AbstractChatCompletionRunner_resolveConnectedPromise, resolve2, "f");
      __classPrivateFieldSet(this, _AbstractChatCompletionRunner_rejectConnectedPromise, reject, "f");
    }), "f");
    __classPrivateFieldSet(this, _AbstractChatCompletionRunner_endPromise, new Promise((resolve2, reject) => {
      __classPrivateFieldSet(this, _AbstractChatCompletionRunner_resolveEndPromise, resolve2, "f");
  ...
  [(_ChatCompletionStream_currentChatCompletionSnapshot = /* @__PURE__ */ new WeakMap(), _ChatCompletionStream_instances = /* @__PURE__ */ new WeakSet(), _ChatCompletionStream_beginRequest = function _ChatCompletionStream_beginRequest3() {
    ...
    for (const { delta, finish_reason, index, logprobs = null, ...other } of chunk.choices) {
      ...
      if (logprobs) {
        ...
        (_a4 = choice.logprobs).token_ids ?? (_a4.token_ids = []);
...
function upload(client, fileName, purpose, check = true) {
  return new APIPromise(client, new Promise(async (resolve2, reject) => {
    let fileSize = 0;
    try {
...
var APIPromise2 = class _APIPromise extends Promise {
  ...
  _thenUnwrap(transform) {
    return new _APIPromise(__classPrivateFieldGet2(this, _APIPromise_client2, "f"), this.responsePromise, async (client, props) => addRequestID(transform(await this.parseResponse(client, props), props), props.response));
  }
  /**
   * Gets the raw `Response` instance instead of parsing the response
   * data.
...
var AbstractChatCompletionRunner2 = class extends EventStream {
  ...
  async finalFunctionToolCall() {
    await this.done();
    return __classPrivateFieldGet2(this, _AbstractChatCompletionRunner_instances2, "m", _AbstractChatCompletionRunner_getFinalFunctionToolCall).call(this);
  }
  async finalFunctionToolCallResult() {
    await this.done();
    return __classPrivateFieldGet2(this, _AbstractChatCompletionRunner_instances2, "m", _AbstractChatCompletionRunner_getFinalFunctionToolCallResult).call(this);
  }
  async totalUsage() {
    await this.done();
  ...
  _emitFinal() {
    ...
    if (finalContent)
      this._emit("finalContent", finalContent);
    const finalFunctionCall = __classPrivateFieldGet2(this, _AbstractChatCompletionRunner_instances2, "m", _AbstractChatCompletionRunner_getFinalFunctionToolCall).call(this);
    if (finalFunctionCall)
      this._emit("finalFunctionToolCall", finalFunctionCall);
    const finalFunctionCallResult = __classPrivateFieldGet2(this, _AbstractChatCompletionRunner_instances2, "m", _AbstractChatCompletionRunner_getFinalFunctionToolCallResult).call(this);
    if (finalFunctionCallResult != null)
...
var ChatCompletionStream2 = class _ChatCompletionStream extends AbstractChatCompletionRunner2 {
  ...
  [(_ChatCompletionStream_params = /* @__PURE__ */ new WeakMap(), _ChatCompletionStream_choiceEventStates = /* @__PURE__ */ new WeakMap(), _ChatCompletionStream_currentChatCompletionSnapshot2 = /* @__PURE__ */ new WeakMap(), _ChatCompletionStream_instances2 = /* @__PURE__ */ new WeakSet(), _ChatCompletionStream_beginRequest2 = function _ChatCompletionStream_beginRequest3() {
    ...
    for (const choice of chunk.choices) {
      ...
      const state = __classPrivateFieldGet2(this, _ChatCompletionStream_instances2, "m", _ChatCompletionStream_getChoiceEventState).call(this, choiceSnapshot);
      if (choiceSnapshot.finish_reason) {
        __classPrivateFieldGet2(this, _ChatCompletionStream_instances2, "m", _ChatCompletionStream_emitContentDoneEvents).call(this, choiceSnapshot);
        if (state.current_tool_call_index != null) {
          __classPrivateFieldGet2(this, _ChatCompletionStream_instances2, "m", _ChatCompletionStream_emitToolCallDoneEvent).call(this, choiceSnapshot, state.current_tool_call_index);
        }
      }
    ...
    if (choiceSnapshot.message.content && !state.content_done) {
      ...
      const responseFormat = __classPrivateFieldGet2(this, _ChatCompletionStream_instances2, "m", _ChatCompletionStream_getAutoParseableResponseFormat).call(this);
    ...
    __classPrivateFieldSet2(this, _ChatCompletionStream_currentChatCompletionSnapshot2, void 0, "f");
    __classPrivateFieldSet2(this, _ChatCompletionStream_choiceEventStates, [], "f");
    return finalizeChatCompletion2(snapshot, __classPrivateFieldGet2(this, _ChatCompletionStream_params, "f"));
    ...
    for (const { delta, finish_reason, index, logprobs = null, ...other } of chunk.choices) {
      ...
      if (content) {
        choice.message.content = (choice.message.content || "") + content;
        if (!choice.message.refusal && __classPrivateFieldGet2(this, _ChatCompletionStream_instances2, "m", _ChatCompletionStream_getAutoParseableResponseFormat).call(this)) {
...
Completions3.Messages = Messages;
...
Jobs2.Checkpoints = Checkpoints2;
...
FineTuning2.Methods = Methods;
FineTuning2.Jobs = Jobs2;
FineTuning2.Checkpoints = Checkpoints;
FineTuning2.Alpha = Alpha;
...
var Files4 = class extends APIResource2 {
  ...
  content(fileID, params, options) {
    const { vector_store_id } = params;
    return this._client.getAPIList(path2`/vector_stores/${vector_store_id}/files/${fileID}/content`, Page, { ...options, headers: buildHeaders2([{ "OpenAI-Beta": "assistants=v2" }, options?.headers]) });
  }
};
...
var OpenAI = class {
   ...
   */
  constructor({ baseURL: baseURL2 = readEnv2("OPENAI_BASE_URL"), apiKey = readEnv2("OPENAI_API_KEY"), organization = readEnv2("OPENAI_ORG_ID") ?? null, project = readEnv2("OPENAI_PROJECT_ID") ?? null, webhookSecret = readEnv2("OPENAI_WEBHOOK_SECRET") ?? null, ...opts } = {}) {
  ...
  buildURL(path3, query, defaultBaseURL) {
    const baseURL2 = !__classPrivateFieldGet2(this, _OpenAI_instances, "m", _OpenAI_baseURLOverridden).call(this) && defaultBaseURL || this.baseURL;
    const url = isAbsoluteURL2(path3) ? new URL(path3) : new URL(baseURL2 + (baseURL2.endsWith("/") && path3.startsWith("/") ? path3.slice(1) : path3));
    const defaultQuery = this.defaultQuery();
...
OpenAI.Files = Files3;
OpenAI.Images = Images2;
OpenAI.Audio = Audio2;
OpenAI.Moderations = Moderations;
OpenAI.Models = Models2;
OpenAI.FineTuning = FineTuning2;
...
var __dirname = (0, import_path40.dirname)(__filename);
dotenv.config({ path: (0, import_path40.resolve)(__dirname, "../.env") });
var SAMPLE_TEXT = `So im Blick. So im Blick. So im Blick. Also etwas haben wir jetzt vorhin noch gesagt. F\xFCrs Protokoll muss ich noch etwas sagen. F\xFCrs Protokoll. Das ist echt mega. Das ist... Und dazu, obwohl du auch, deine Erfahrung, also deine Erfahrung f\xFCr dich. Das ist unterst\xFCtzung von dir gef\xFChlt bez\xFCglich mir. Das und das andere ist, dass ich es mega sch\xF6n gefunden habe, dass du am Konzert dabei gewesen bist. Und eben so vorne gestanden bist und... und dabei gewesen bist. Und auch wenn es nicht alles so jetzt dieses war, dass ich auch dabei gewesen bin. Ja, habe ich wirklich mega gesch\xE4tzt. Und \xE4h... Und dann ist mir noch mal etwas in den Sinn gekommen. Und was ich auch mega gesch\xE4tzt habe, dass ich jetzt gerade am Dienstag, als ich so den Abschluss hatte und... das Gef\xFChl hatte, wenn ich mehr etwas mitteilen will oder etwas brauche von dir oder du bist einfach mega da. Du bist sehr pr\xE4sent. Und dass ich dann den Raum bekomme, um mich mitzuteilen oder zu br\xFCllen oder was auch gerade Emotionen oder was gerade aufkommt. Ja, das ist mega sch\xF6n.`;
var SYSTEM_PROMPT = `Du bist ein Assistent, der pr\xE4gnante, aussagekr\xE4ftige Titel f\xFCr Tagebucheintr\xE4ge generiert. Der Titel soll kurz (maximal 5-7 W\xF6rter), informativ und ansprechend sein. Antworte NUR mit dem Titel, ohne zus\xE4tzliche Erkl\xE4rungen oder Anf\xFChrungszeichen.

Datum: 10. Februar 2026
Eintragstyp: Allgemein`;
async function testTogether(modelId, maxTokens = 100) {
  ...
  try {
    ...
    const response = await together.chat.completions.create({
      ...
      messages: [
        ...
        { role: "user", content: SAMPLE_TEXT.substring(0, 1e3) }
      ],
